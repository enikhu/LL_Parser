...
def first_comp():
    ...
    nterm = re.compile(r'[A-Z]~?')
    term1 = list(set(term.findall(g.get(1.0,END).strip())))
    nterm1 = list(set(nterm.findall(g.get(1.0,END).strip())))
    for i in nterm1:
        str = str + i+ ','
    str = str[:-1]
    nterm1 = str
...
#great = Text(width="21", height="10",font='Arial -24')
#great.place(relx = 0.50, rely = 0.15)
...
s.insert(END,"Enter String to be Parsed")
